...
class web3go: NSObject {
  private let client = Web3goNewEthereumClient("http://119.3.43.136:8203", nil)
  private let ctx = Web3goNewContext()
  private let path = "m/44'/60'/0'/0/0";
  ...
  // generate hdwallet
  @objc
  func generateHdWallet(_ callback: RCTResponseSenderBlock) {
    let mnemonic: String = Web3goNewMnemonic(128, nil);
    let hdwallet: Web3goWallet! = Web3goNewFromMnemonic(mnemonic, nil);
    let account: Web3goNewAccount!;
    let address: String;
    let privateKey: String;
    let publicKey: String;
    do {
      try account = hdwallet?.derive(path, pin: false);
      try address = (hdwallet?.addressHex(account) ?? nil) ?? "0xxxx";
      try privateKey = (hdwallet?.privateKeyHex(account) ?? nil) ?? "0xxxx";
      try publicKey = (hdwallet?.publicKeyHex(account) ?? nil) ?? "0xxxx";
      let result = "\(mnemonic)&&\(address)&&\(privateKey)&&\(publicKey)"
      callback([result])
    } catch {
  ...
  func getBalance(_ address: String, callback: RCTResponseSenderBlock) {
    ...
    do {
      ...
      callback([ammount.getString(10) ?? 0])
    } catch {
      callback(["error"])
    }
  }
